...
def init_trial_path(args, exp_configure):
    ...
    while path_exists:
        ...
        if args['model'] == 'GCN':
            model_folder = args['model']+'-' \
                      +str(exp_configure['lr'])+'-' \
                      +str(exp_configure['batch_size'])+'-' \
                      +str(exp_configure['dropout'])+'-' \
                      +str(exp_configure['gnn_hidden_feats'])+'-' \
                      +str(exp_configure['predictor_hidden_feats'])+'-' \
                      +str(exp_configure['num_gnn_layers'])+'_' \
                      +datetime.datetime.now().strftime("%M")
        if args['model'] == 'MPNN':
            model_folder = args['model']+'-' \
                      +str(exp_configure['lr'])+'-' \
                      +str(exp_configure['batch_size'])+'-' \
                      +str(exp_configure['node_out_feats'])+'-' \
                      +str(exp_configure['edge_hidden_feats'])+'-' \
                      +str(exp_configure['num_step_message_passing'])+'-' \
                      +str(exp_configure['num_step_set2set'])+'-' \
                      +str(exp_configure['num_layer_set2set'])+'_' \
                      +datetime.datetime.now().strftime("%M")

        path_to_results = os.path.join(args['result_path'], model_folder)
