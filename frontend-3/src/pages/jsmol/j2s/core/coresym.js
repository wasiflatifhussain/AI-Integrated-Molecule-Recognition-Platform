...
Clazz_load (["JU.P3", "$.SB"], "J.adapter.smarter.XtalSymmetry", ["java.lang.Boolean", "$.Character", "$.Float", "java.util.Hashtable", "JU.BS", "$.Lst", "$.M3", "$.M4", "$.P3i", "$.PT", "$.V3", "J.adapter.smarter.Atom", "JS.Symmetry", "$.SymmetryOperation", "JU.BSUtil", "$.SimpleUnitCell"], function () {
...
if (this.acr.noPack ? !this.removePacking (this.ndims, pt0, 0, 1, 0, 1, 0, 1, this.packingError) : !this.isWithinCell (this.ndims, pt0, 0, 1, 0, 1, 0, 1, this.packingError)) bsAtoms.clear (i);
}
return;
}var offset = null;
this.nVib = 0;
...
this.setMinMax (dim, kcode, maxX, maxY, maxZ);
...
if (!this.removePacking (this.ndims, atoms[i], this.minXYZ.x, this.maxXYZ.x, this.minXYZ.y, this.maxXYZ.y, this.minXYZ.z, this.maxXYZ.z, this.packingError)) bs.clear (i);
}
} else {
for (var i = bs.nextSetBit (iAtomFirst); i >= 0; i = bs.nextSetBit (i + 1)) {
if (!this.isWithinCell (this.ndims, atoms[i], this.minXYZ.x, this.maxXYZ.x, this.minXYZ.y, this.maxXYZ.y, this.minXYZ.z, this.maxXYZ.z, this.packingError)) bs.clear (i);
}
}}, "~N");
...
this.ndims = Clazz_floatToInt (this.symmetry.getUnitCellInfoType (6));
...
sym.toFractional (pttemp, false);
if (this.acr.fixJavaFloat) JU.PT.fixPtFloats (pttemp, 100000.0);
if (!this.isWithinCell (this.ndims, pttemp, this.minXYZ0.x, this.maxXYZ0.x, this.minXYZ0.y, this.maxXYZ0.y, this.minXYZ0.z, this.maxXYZ0.z, this.packingError)) continue;
...
"PARTICLE_CHAIN", 1,
"PARTICLE_SYMOP", 2,
"MAX_INTERCHAIN_BOND_2", 25,
"mident", null);
});
Clazz_declarePackage ("JS");
Clazz_load (["J.api.SymmetryInterface"], "JS.Symmetry", ["JU.BS", "$.Lst", "$.P3", "$.PT", "J.api.Interface", "J.bspt.Bspt", "JS.PointGroup", "$.SpaceGroup", "$.SymmetryInfo", "$.SymmetryOperation", "$.UnitCell", "JU.Escape", "$.Logger", "$.SimpleUnitCell"], function () {
c$ = Clazz_decorateAsClass (function () {
...
c$.mInv = c$.prototype.mInv = JU.M3.newA9 ( Clazz_newFloatArray (-1, [-1, 0, 0, 0, -1, 0, 0, 0, -1]));
});
Clazz_declarePackage ("JS");
...
if (!haveExtension || ext.charAt (0) == '?') for (i = 0; i < lastIndex; i++) if (((s = JS.SpaceGroup.SG[i]).hmSymbolAbbr.equalsIgnoreCase (abbr) || s.hmSymbolAbbrShort.equalsIgnoreCase (abbr) || s.intlTableNumber.equals (abbr)) && (!checkBilbao || s.isBilbao)) switch (s.ambiguityType) {
case '\0':
return i;
case 'a':
...
if (Clazz_instanceOf (this.info, String)) return null;
var map = this.info;
...
Clazz_declarePackage ("JS");
Clazz_load (["JU.M4"], "JS.SymmetryOperation", ["java.lang.Boolean", "$.Character", "$.Float", "java.util.Hashtable", "JU.Lst", "$.Matrix", "$.P3", "$.PT", "$.SB", "$.V3", "JS.Symmetry", "JU.Logger", "$.Parser"], function () {
c$ = Clazz_decorateAsClass (function () {
...
function (xyz) {
try {
return JS.SymmetryOperation.getMatrixFromString (null, xyz, null, false);
} catch (e) {
if (Clazz_exceptionOf (e, Exception)) {
return null;
} else {
...
if (linearRotTrans != null) linearRotTrans[linearRotTrans.length - 1] = 1;
...
mat.getRow (i, row);
var term = "";
for (var j = 0; j < 3; j++) if (JS.SymmetryOperation.approxF (row[j]) != 0) term += JS.SymmetryOperation.plusMinus (term, row[j], JS.SymmetryOperation.labelsXYZ[j + lpt]);

if (JS.SymmetryOperation.approxF (row[3]) != 0) term += JS.SymmetryOperation.xyzFraction12 ((is12ths ? row[3] : row[3] * denom), denom, allPositive, halfOrLess);
str += "," + term;
}
...
Clazz_declarePackage ("JS");
Clazz_load (["JU.P3", "$.V3"], "JS.SymmetryDesc", ["java.lang.Float", "java.util.Hashtable", "JU.BS", "$.Lst", "$.M4", "$.Measure", "$.P4", "$.PT", "$.Quat", "$.SB", "JM.Atom", "JS.T", "JS.SpaceGroup", "$.Symmetry", "$.SymmetryOperation", "JU.Escape", "$.Logger"], function () {
c$ = Clazz_decorateAsClass (function () {
this.modelSet = null;
Clazz_instantialize (this, arguments);
}, JS, "SymmetryDesc");
...
case 36868:
return (io[5] != null ? "none" : io[6] != null ? io[6] : io[8] != null ?  Clazz_newArray (-1, [io[7], io[8]]) : io[14] != null ? io[14] : "identity");
}
}, "~A,~N");
...
if (haveInversion) {
type = info1 = (Clazz_doubleToInt (360 / ang)) + "-bar axis";
} else if (pitch1 != 0) {
type = info1 = (Clazz_doubleToInt (360 / ang)) + "-fold screw axis";
JS.SymmetryDesc.ptemp.setT (ax1);
uc.toFractional (JS.SymmetryDesc.ptemp, false);
info1 += "|translation: " + JS.SymmetryDesc.strCoord (op, JS.SymmetryDesc.ptemp, op.isBio);
} else {
type = info1 = "C" + (Clazz_doubleToInt (360 / ang)) + " axis";
}} else if (trans != null) {
...
c$.approx0f = Clazz_defineMethod (c$, "approx0f", 
 function (x) {
...
if (asString) {
return ret1;
}if (Clazz_instanceOf (ret1, String)) return nullRet;
info = ret1;
if (type == 1140850689) {
...
"RET_LIST", 21,
"RET_INVARIANT", 22,
"keys",  Clazz_newArray (-1, ["xyz", "xyzOriginal", "label", null, "fractionalTranslation", "cartesianTranslation", "inversionCenter", null, "axisVector", "rotationAngle", "matrix", "unitTranslation", "centeringVector", "timeReversal", "plane", "_type", "id", "cif2", "xyzCanonical", "xyzNormalized"]));
